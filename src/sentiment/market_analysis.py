...
def analyze_market_trends():
    ...
    try:
        ...
        for source in sentiment_sources:
            try:
                with open(DATA_DIR / f"{source}_sentiment.json", "r") as f:
                    sentiment_data[source] = json.load(f)
            except FileNotFoundError:
                logging.error(f"File not found: {os.path.abspath(DATA_DIR / f'{source}_sentiment.json')}")
                sentiment_data[source] = None

        # Load market data
        logging.info("Loading market data from file...")
        try:
            with open(DATA_DIR / "market_data.json", "r") as f:
        ...
        # Save market trends
        with open(DATA_DIR / "market_trends.json", "w") as f:
